...
public class Tree4Steps<E extends Comparable<? super E>> implements ThreeForm<E> {
    private int size;
    ...
    public void add(E value) {
        ...
        if (howManySteps(parent,0)>3) return;
    ...
    private int howManySteps (NodWithParent<E> nod, int step){
        if (nod.data.compareTo(root.data)==0){
            return 1;
        }
        return howManySteps(nod.getParent(), step)+1;
    }
    ...
    private NodWithParent<E> findParentOfNod (NodWithParent<E> nod, E value){
        if(value.compareTo(nod.getData())==0) return nod;
            ...
            if(nod.getRightChild()==null) return nod;
                ...
                return findParentOfNod(nod.getRightChild(), value);
            }
        }else if(value.compareTo(nod.data)<0) {
            if(nod.getLeftChild()==null) return nod;
            else {
                return findParentOfNod(nod.getLeftChild(), value);
            }
        }
        throw new IllegalArgumentException("SWW");
    }
    ...
    private boolean isTreeBalanced(NodWithParent<E> nod){
        if (nod==null) return true;
        int balanceOfChild = heightOfChild(nod.getLeftChild())-heightOfChild(nod.getRightChild());
        if (balanceOfChild<-1||balanceOfChild>1) return false;
        return isTreeBalanced(nod.getRightChild()) && isTreeBalanced(nod.getLeftChild());
    }
    private int heightOfChild (NodWithParent<E> nod){
        if(nod==null) return 0;
        int maxHeight;
        if (heightOfChild(nod.getRightChild())>heightOfChild(nod.getLeftChild())) {
            maxHeight = heightOfChild(nod.getRightChild());
        } else {
            maxHeight = heightOfChild(nod.getLeftChild());
        }
        int returnValue = 1+maxHeight;
        return returnValue;
    }
